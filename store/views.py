...
from store.services.get_home import get_dict_all_products_like, \
    get_dict_query_products_like, get_input_search_query, get_order_dict, get_path_to_redirect
...
def likeView(request, product_id, post_id, cat):
    ...
    if cat == '0':
        ...
        response = redirect(reverse('search_products'),
                            get_input_search_query(request))
    else:
        response = HttpResponseRedirect(
...
class Category(DataMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        c_def = self.get_user_context(
            order=self.choice,
            category=Product.objects.filter(type_of_product__slug=slugify(
                category_slug)).first().type_of_product,
